...
export async function validateToken(token: string): Promise<StaffUser | null> {
  try {
    const decoded = jwt.verify(token, config.jwt.secret) as JwtPayload & { userId: string };
    const result = await query<StaffUser>(
      'SELECT id, employee_number, first_name, last_name, display_name, email, phone, role, status, timezone, metadata, created_at, updated_at FROM staff_users WHERE id = $1 AND status = $2',
